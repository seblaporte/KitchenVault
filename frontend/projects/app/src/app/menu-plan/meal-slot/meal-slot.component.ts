...
import { heroSparkles, heroShoppingCart, heroCheck, heroArrowsRightLeft } from '@ng-icons/heroicons/outline';
import { MealPlanEntryDto } from '@KitchenVault/api-client';

export type HeldMode = 'none' | 'source' | 'target-empty' | 'target-swap';

@Component({
  selector: 'app-meal-slot',
  standalone: true,
  imports: [CommonModule, RouterLink, NgIconComponent],
  providers: [provideIcons({ heroSparkles, heroShoppingCart, heroCheck, heroArrowsRightLeft })],
  template: `
    @if (entry && heldMode === 'target-swap') {
      <button
        (click)="onPlace()"
        class="group relative w-full text-left rounded-xl border-2 border-amber-400 dark:border-amber-500 bg-white dark:bg-stone-900 overflow-hidden cursor-pointer"
        [attr.aria-label]="'Échanger avec ' + entry.recipeName"
      >
        <span class="absolute top-1.5 right-1.5 z-10 flex items-center justify-center h-5 px-1.5 rounded-md bg-amber-100 dark:bg-amber-950/60 text-amber-700 dark:text-amber-400 text-[11px] font-semibold">⇄</span>
        ...
        <div class="p-2.5 flex flex-col gap-1">
          <p class="text-xs font-medium text-stone-800 dark:text-stone-200 line-clamp-2 leading-snug">{{ entry.recipeName }}</p>
        </div>
      </button>
    } @else if (entry) {
      <div
        class="group relative rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-900 overflow-hidden hover:-translate-y-0.5 hover:border-stone-300 dark:hover:border-stone-600 hover:shadow-md transition-all"
        [class.opacity-30]="heldMode === 'source'"
        [class.pointer-events-none]="heldMode === 'source'"
      >
        <div class="w-full h-24 bg-stone-100 dark:bg-stone-800 overflow-hidden flex items-center justify-center">
          @if (entry.recipeThumbnailUrl) {
            <img [src]="entry.recipeThumbnailUrl" [alt]="entry.recipeName" class="w-full h-full object-cover" loading="lazy" />
    ...
    } @else if (heldMode === 'target-empty') {
      <button
        (click)="onPlace()"
        class="w-full min-h-[130px] rounded-xl border-2 border-dashed border-amber-400 dark:border-amber-500 bg-amber-50/60 dark:bg-amber-950/20 flex flex-col items-center justify-center gap-1.5 cursor-pointer animate-pulse"
        [attr.aria-label]="'Déplacer la recette ici — ' + label"
      >
        <svg class="h-4 w-4 text-amber-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
        </svg>
        <span class="text-xs font-medium text-amber-600 dark:text-amber-400">Déposer ici</span>
      </button>
    } @else {
...
export class MealSlotComponent {
  ...
  @Output() moveRequested = new EventEmitter<{ date: string; mealType: string }>();
  @Output() placeRequested = new EventEmitter<{ date: string; mealType: string }>();
  ...
  onMove(): void {
    this.moveRequested.emit({ date: this.date, mealType: this.mealType });
  }

  onPlace(): void {
    this.placeRequested.emit({ date: this.date, mealType: this.mealType });
  }
